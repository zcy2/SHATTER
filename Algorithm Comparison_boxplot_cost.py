...
for net in networks:
    ...
    # Import Data数据是从EMDA_GMCC等程序中得来，由于前期失误，所有数据需要统一除以网络层数3
    df1 = pd.read_csv('./save/network_'+net+'_90-150/Algorithm Comparison_cost.csv')
    # df1['ANC']= df1['ANC'].apply(lambda x: x / 3)
    df2 = pd.read_csv('./save/network_'+net+'_150-300/Algorithm Comparison_cost.csv')
    # df2['ANC']= df2['ANC'].apply(lambda x: x / 3)
    df3 = pd.read_csv('./save/network_'+net+'_300-600/Algorithm Comparison_cost.csv')
    # df3['ANC']= df3['ANC'].apply(lambda x: x / 3)
    df4 = pd.read_csv('./save/network_'+net+'_600-900/Algorithm Comparison_cost.csv')
    ...
    # plt.show()
    plt.savefig('./save/Algorithm_Comparison_cost_'+net+'.png', format='png', bbox_inches='tight')